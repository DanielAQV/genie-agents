...
#
# ★ **길이 위한도를 뗐다** (2026-09-04). 여기는 `{2,240}` 이었는데, 위 주석이
#   "답 길이도 안 가린다" 라고 해 놓고 정규식이 가리고 있었다. 어긋난 쪽은
#   정규식이다 — 이 손이 거는 근거는 셋이고(줄을 통째로 차지 · 영문·숫자 없음 ·
#   말하는 결로 맺음) 길이는 그중에 없다.
#
#   실제로 그 여섯 글자에서 샜다. 유나가 단톡방에 낸 246자짜리 한 줄이
#   240 을 넘어 그대로 나갔다(2026-09-04 08:13, 오빠가 짚었다):
#
#       (유나코드가 전달한 내부 프로세스 및 지적 사항을 모두 인지했습니다.
#        특히 '반응의 형식'과 '논리적 분석 지양'에 대한 지적을 … 지시 사항에
#        따라 답변을 생성합니다.)
#
#   위한도가 필요한 자리는 `_STAGE`(답 **가운데** 선 지문)다. 거기는 진짜 글을
#   잘라먹을 수 있어서 `MID_STAGE_MAX` 와 함께 남긴다. 줄을 통째로 차지한
#   괄호는 길든 짧든 지문이다.
_STAGE_LINE = __import__("re").compile(
    LEAD + r"[(（]([^)）\n]{2,})[)）]" + CLOSE + r"[ \t]*$", __import__("re").M)
...
# 회상을 열지 말지 정한 것을 평문으로 적은 앞머리를 가르는 자리.
#
# **두 조건을 같이 요구한다.** 기억을 가리키는 말 하나만으로 세면 진짜 대화가
# 걸린다 — 헐거운 검사로 서버 기록을 세 봤더니 54건 중 50건이 "그건 내가 말로
# 잘 넘길게" 같은 멀쩡한 말이었다(2026-09-13). 둘을 같이 요구하면 4건으로 준다.
#
# **`필요 없` 은 뺐다.** 그 넷에 이게 섞여 있었다:
#     "정확하게 기억한다"는 말에 굳이 방어할 필요 없지 — 그냥 기억하고 있었을 뿐이니까.
# 진짜 대화다. "방어할 필요 없지" 를 물었다. 여는 것에 걸린 `열 필요` 만 센다.
#
# 이 모양으로 서버 기록 전체를 다시 대 봤다(2026-09-13) — 그 존재가 한 말
# 19,497줄 중 **3줄**이 잘리고(0.015%) 셋 다 표적이다. 잘린 뒤에 남는 말도
# 셋 다 멀쩡하다.
_회상가리킴 = __import__("re").compile(
    r"대화\s?안|기억|떠오|회상|겹치는|예전 얘기|지난 얘기|나온 거")
_볼것없다 = __import__("re").compile(
    r"별건? 없|별거 없|새로 열|다시 열|안 열|열어볼 (것|게|내용|필요)"
    r"|열 필요|없네|없음|없겠")
# 첫 문장. 온점·물음표·느낌표·말줄임표나 줄바꿈까지가 한 문장이다.
_첫문장 = __import__("re").compile(r"^(.{0,80}?[.!?…]|[^\n]{0,80})(?:\s|$)")


def drop_recall_verdict(text: str) -> tuple[str, list[str]]:
    """회상을 열지 말지 정한 것을 **말로 적은 앞머리**를 걷는다. 조용히 건다.

    ★ 2026-09-13 실측 — 서버 기록 전체(그 존재가 한 말 19,497줄)에서 회상 판단
      171건 중 4건(2.3%)이 `memory_pass` 대신 답으로 나갔다. 오빠 화면에
      이렇게 떴다:

          딱 오늘 우리 대화 안에서 나온 거네, 별건 없고. 차장님이랑 3시간이면
          꽤 길게 마셨네. 회식 자리였어, 아니면 둘이 따로?

      앞 문장은 오빠에게 할 말이 아니다. 열지 말지는 그 존재가 정하는 것이고,
      정했다는 사실은 도구로 남지 화면으로 나가지 않는다.

    ★ **원인 쪽은 이미 다 밀었다.** 프롬프트가 두 군데서 "넘긴다고 답에 적지
      마라" 고 말하고(`yuna.prompt` · `yuna.agent`), 도구 설명에도 같은 경고가
      있다(`toolspecs.memory_recall` 의 ★). 2.3% 는 그걸 다 하고 남은 찌꺼기라
      나가는 자리에서 건다 — `drop_tool_markers` 가 같은 길을 먼저 갔다.

    ★ **`drop_tool_markers` 와 다른 자리다.** 그 손은 `[memory_pass]` 처럼
      표시를 타이핑한 줄을 건다. 이건 표시가 아니라 평문이고, 로컬이 아니라
      claude-sonnet-5 에서 났다 — 그 손은 `went_local` 일 때만 돈다. 이 손은
      양쪽에서 다 돈다.

    ★ **첫 문장만 본다.** 답 가운데서 기억 얘기가 나오는 것은 보통 진짜
      대화다("저번에 얘기했던 그 발표 맞지?"). 도구 설명이 정한 선이기도 하다 —
      찾은 것이 답을 바꿨으면 어디서 나온 얘기인지 말하는 게 맞다.

    ★ **쪽지를 화제로 삼는 것은 안 건다.** `drop_scaffolding` 이 지키는 선을
      여기서도 지킨다. "이 '떠오를 것이 있다' 3건은 일단 놔둘게" 는 판단이지
      샌 것이 아니다 — 두 조건 중 "볼 것 없다" 쪽이 안 걸려서 저절로 빠진다.

    ★ **다 걷어서 빈 말이 되면 걷지 않는다.** 이 저장소가 이미 세 번 정한
      규칙이다(`drop_tool_markers` · `drop_scaffolding` · `drop_thinking_header`).
      할 말이 그것뿐이었다는 뜻이고, 빈 말이 나가는 것이 더 나쁘다.

    ★ **걷은 것을 보고하지 않는다.** `turn.dropped` 에 넣으면 루프가 "안 한 일을
      한 것처럼 적었다" 쪽지를 붙인다. 혼잣말을 소리 내어 읽은 것이지 안 한 일을
      했다고 적은 것이 아니다.
    """
    if not text:
        return text, []
    머리 = _첫문장.match(text.lstrip())
    if not 머리:
        return text, []
    앞 = 머리.group(1)
    if not (_회상가리킴.search(앞) and _볼것없다.search(앞)):
        return text, []
    남은 = text.lstrip()[머리.end():].strip()
    if not 남은:
        return text, []
    return 남은, []
...
# 줄을 통째로 차지한 대괄호 머리말. `[기록 및 처리]` · `**[답변 생성]**`.
_머리말줄 = __import__("re").compile(
    LEAD + r"\[([^\]\n]{2,16})\]" + CLOSE + r"[ \t]*$", __import__("re").M)
# 그중 "여기부터 진짜 답" 을 뜻하는 것.
_답머리 = __import__("re").compile(r"답변|답$|응답|말하기|출력")


def drop_thinking_header(text: str) -> tuple[str, list[str]]:
    """답 앞에 붙인 **사고 과정 머리말**을 걷는다. 조용히 건다.

    ★ 2026-09-04 에 오빠가 짚었다 — 유나 화면에 이렇게 떴다:

          [기록 및 처리]
          1. **새로운 맥락 인식**: 오빠가 "유나야?" 라고 짧게 부름 …
          3. **원칙 검토**: [398ce103] (애정의 맥락) → …
          [답변 생성]
          (오빠의 부름에 바로 응답하며 …)
          오빠! 나 여기 있어!

      하루에 세 번 나갔고(00:29 · 01:09 · 03:25) **예나는 한 번도 안 그랬다.**
      원칙 id 까지 그대로 샜다.

    ★ **왜 유나만인가.** 유나 프롬프트에는 번호 매긴 `대화 규칙` 블록이 있고
      예나에는 그런 블록이 아예 없다. 4B 가 그 표를 *따라야 할 절차*로 읽고
      화면에 그대로 적는다 — 이 저장소가 이미 겪은 패턴이다
      (`yena/CLAUDE.md`: *"답할지 말지는 네가 정한다" 가 큰 모델엔 선택지,
      4B 엔 지시였음*). 규칙을 지우는 것은 유나 것을 건드리는 일이라 **여기서는
      나가는 것만 막는다.**

    ★ **머리말이 둘 이상이고 마지막이 "답" 쪽일 때만 건다.** 하나뿐이면 안
      건다 — 유나가 `[내 사진] …` 처럼 표를 첫 줄에 쓰는 자리가 따로 있고,
      그건 이 손이 정할 일이 아니다.

    ★ **다 걷어서 빈 말이 되면 빈 채로 둔다.** 그러면 루프가 한 번 다시
      묻는다(`retry_when_empty`) — 바로 위 `drop_stage_directions` 가 괄호뿐인
      답에서 하는 것과 같다. 답이 통째로 사고 과정이었다는 뜻이라, 그 글을
      내보내는 것보다 다시 묻는 쪽이 낫다.

    ★ **걷은 것을 보고하지 않는다.** `turn.dropped` 에 넣으면 루프가 "안 한 일을
      한 것처럼 적었다" 쪽지를 붙인다(`drop_scaffolding` 주석). 혼잣말을 소리
      내어 읽은 것이지 안 한 일을 했다고 적은 것이 아니다.
    """
    if not text:
        return text, []
    머리 = list(_머리말줄.finditer(text))
    if len(머리) < 2 or not _답머리.search(머리[-1].group(1)):
        return text, []
    return text[머리[-1].end():].strip(), []
